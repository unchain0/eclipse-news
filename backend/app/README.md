# Prisma News – Backend

API backend para o projeto **Prisma News**, responsável por:

- Fazer **scraping periódico** de notícias de vários portais.
- Persistir sites e notícias em **PostgreSQL**.
- Expor endpoints REST em **FastAPI** para o frontend Angular consumir.

## Stack

- **Python** 3.13
- **FastAPI**
- **SQLAlchemy** (ORM)
- **Alembic** (migrações)
- **PostgreSQL**
- **loguru** (logging)

## Pré‑requisitos

- Python instalado (versão compatível com o projeto; recomendado 3.11+).
- PostgreSQL rodando localmente (ou via Docker Compose).
- Ambiente virtual criado e dependências instaladas (ex.: `uv sync` ou `pip install -r requirements.txt`, conforme seu fluxo atual).

## Configuração

1. Crie o banco de desenvolvimento (se ainda não existir), por exemplo:

   ```sql
   CREATE DATABASE prisma_news_dev;
   ```

2. Configure o arquivo `.env` na raiz do projeto com pelo menos:

    ```env
    DATABASE_URL=postgresql://user:password@localhost:5432/prisma_news_dev
    SCRAPE_INTERVAL_SECONDS=60
    ALLOWED_ORIGINS=http://localhost:4200
    ```

3. Rode as migrações do banco:

   ```bash
   alembic upgrade head
   ```

## Executando o backend

Para iniciar o servidor FastAPI com recarregamento automático:

```bash
uvicorn app.main:app --reload --reload-dir app --reload-dir migrations
```

Ao subir, o backend:

- Inicia uma thread de **scraping em background**, que roda em loop usando o intervalo configurado em `SCRAPE_INTERVAL_SECONDS`.
- Popula/garante os registros de `sites` e insere novas `news` respeitando a restrição `UNIQUE(site_id, url)` no banco de dados.

### Endpoints principais

- `GET /health` – verificação simples de saúde da API.
- `GET /sites` – lista todos os sites cadastrados (schema `SiteOut`).
- `GET /news` – lista notícias paginadas, com filtros por site via query params:
  - `sites`: lista de slugs separados por vírgula (ex.: `veja,globo,cnn`).
  - `page`: página (default `1`).
  - `page_size`: tamanho da página (default `20`, máx `100`).

Exemplo:

```bash
curl "http://localhost:8000/news?sites=veja,globo&page=1&page_size=20"
```
